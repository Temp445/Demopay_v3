/*
  # Rename calculation_type to amount_type in payroll_structure_components

  1. Changes
    - Rename column `calculation_type` to `amount_type` in payroll_structure_components table
    - Update all database functions that reference this field
    - Preserve all existing data and constraints

  2. Affected Functions
    - get_payroll_structure_details
    - insert_pay_structure_component
...
CREATE OR REPLACE FUNCTION public.insert_pay_structure_component(p_amount numeric DEFAULT 0, p_amount_type text DEFAULT 'value'::text, p_editability text DEFAULT 'fixed'::text, p_component_id uuid DEFAULT NULL::uuid, p_component_name text DEFAULT ''::text, p_component_type text DEFAULT 'earning'::text, p_iscustom boolean DEFAULT false, p_percentage numeric DEFAULT 0, p_reference_components text[] DEFAULT ARRAY[]::text[], p_structure_id uuid DEFAULT NULL::uuid, p_tenant_id uuid DEFAULT NULL::uuid, p_is_locked boolean DEFAULT false, p_is_applied_in_calculation boolean DEFAULT true, p_expression text DEFAULT NULL::text, p_expression_ast jsonb DEFAULT NULL::jsonb, p_display_order integer DEFAULT 0)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
