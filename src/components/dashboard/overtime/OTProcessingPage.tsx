        ...
        <div className="border-b border-slate-200 bg-slate-50/50 px-6">
          <nav className="-mb-px flex space-x-8">
            ...
            >
              ...
              Pending OT
              {pendingCount > 0 && (
                <span className="ml-1 bg-amber-100 text-amber-700 py-0.5 px-2 rounded-full text-[10px] font-black">{pendingCount}</span>
              )}
            </button>
            ...
            >
              ...
              Approved OT
              {approvedCount > 0 && (
                <span className="ml-1 bg-emerald-100 text-emerald-700 py-0.5 px-2 rounded-full text-[10px] font-black">{approvedCount}</span>
              )}
            </button>
          </nav>
        </div>
        ...
        <>
          ...
          {activeTab === 'approved' && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-col justify-center relative overflow-hidden">
                <div className="absolute top-0 right-0 p-3 opacity-[0.03]">
                  <Users className="h-16 w-16 text-indigo-600 transform translate-x-3 -translate-y-3" />
                </div>
                ...
                <h3 className="text-2xl font-semibold text-slate-900 mt-1 relative z-10">{filteredEmployeeCount}</h3>
              </div>

              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-col justify-center relative overflow-hidden">
                <div className="absolute top-0 right-0 p-3 opacity-[0.03]">
                  <Clock className="h-16 w-16 text-blue-600 transform translate-x-3 -translate-y-3" />
                </div>
                <div className="relative z-10 flex items-center gap-2 mb-1">
                ...
                <h3 className="text-2xl font-semibold text-slate-900 mt-1 relative z-10">{formatOTDuration(filteredTotalHours)}</h3>
              </div>

              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-col justify-center relative overflow-hidden">
                <div className="absolute top-0 right-0 p-3 opacity-[0.03]">
                  <IndianRupee className="h-16 w-16 text-emerald-600 transform translate-x-3 -translate-y-3" />
                </div>
                <div className="relative z-10 flex items-center gap-2 mb-1">
                ...
                <h3 className="text-2xl font-semibold text-slate-900 mt-1 relative z-10">₹{filteredTotalAmount.toLocaleString('en-IN', { maximumFractionDigits: 2, minimumFractionDigits: 2 })}</h3>
              </div>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              <div className="md:col-span-2" ref={searchRef}>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="block w-full pl-9 pr-8 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500"
                  />
                  {searchTerm && (
                    <button onClick={() => setSearchTerm('')} className="absolute inset-y-0 right-2 flex items-center text-gray-300 hover:text-gray-500">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">
                  <Calendar className="inline h-3 w-3 mr-1" />Period From
                </label>
                <input type="date" value={filterFrom} onChange={e => setFilterFrom(e.target.value)}
                  className="block w-full border border-gray-300 rounded-md py-2 px-3 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500" />
              </div>

              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">
                  <Calendar className="inline h-3 w-3 mr-1" />Period To
                </label>
            ...
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100">
              <div className="flex items-center gap-4">
                <div className="text-sm text-slate-500 font-medium">
                  Showing <span className="font-black text-slate-800">{filtered.length}</span> record{filtered.length !== 1 ? 's' : ''}
                </div>
                  ...
                  <div className="flex items-center gap-2 border-l border-slate-200 pl-4">
                    ...
                    {activeTab === 'pending' ? (
                      <button
                        ...
                        className="px-3 py-1 bg-indigo-600 text-white text-xs font-bold rounded-md hover:bg-indigo-700 transition"
                      >
                        Bulk Approve
                      </button>
                    ) : (
                      <button
                        ...
                        className="px-3 py-1 bg-amber-500 text-white text-xs font-bold rounded-md hover:bg-amber-600 transition"
                      >
                        Bulk Revoke
                      </button>
                    )}
              ...
              {hasFilters && (
                <button onClick={clearFilters} className="flex items-center gap-1.5 text-xs font-bold text-rose-500 hover:text-rose-700">
                          ...
                          {activeTab === 'pending' ? (
                            <>
                              <button
                                ...
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 text-white text-xs font-bold rounded-md hover:bg-emerald-700 transition-colors shadow-sm"
                              >
                                <CheckCircle2 className="h-3.5 w-3.5" />Approve
                              </button>
                              <button
                                onClick={() => handleDelete(record)}
                                className="p-1.5 text-rose-400 hover:text-white hover:bg-rose-500 rounded-md transition-colors border border-rose-100 hover:border-rose-500"
                                title="Delete"
                              >
                            ...
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-500 text-white text-xs font-bold rounded-md hover:bg-amber-600 transition-colors shadow-sm"
                            >
