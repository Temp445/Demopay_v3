...
function DomainConfigWrapper({ children }: { children: React.ReactNode }) {
  const { fetchConfig, initialized } = useDomainConfigStore();

  React.useEffect(() => {
    fetchConfig(window.location.host);
  }, [fetchConfig]);
...
function InitialPageRouter() {
  const { allowToLandingPage } = useDomainConfigStore();

  if (!allowToLandingPage) {
