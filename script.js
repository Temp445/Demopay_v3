const fs = require('fs');
const file = 'c:/Users/nithi/Desktop/Clones/Demopay_v3/src/components/dashboard/location/TravelAllowanceTab.tsx';
let content = fs.readFileSync(file, 'utf8');

const targetStartStr =   const renderJourneyGroup = (group: JourneyGroup, tabType: 'pending' | 'approved' | 'rejected') => {;
const targetEndStr =       {/* --- Bulk Action Floating Bar --- */};
...
const newContent = \  const renderJourneyGroup = (group: JourneyGroup, tabType: 'pending' | 'approved' | 'rejected') => {
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-gray-50">
            <div className="flex items-center gap-4">
              <div className="flex-shrink-0 h-12 w-12 rounded-2xl flex items-center justify-center font-bold text-base shadow-inner \">
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {/* --- Page Header --- */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 relative mb-8 mt-2">
        <div className="relative z-10">
          <div className="absolute -left-6 -top-6 w-24 h-24 bg-blue-400/20 rounded-full blur-2xl" />
          ...
          <p className="text-sm text-gray-500 mt-1.5 font-medium">Review and approve completed work assignments</p>
        </div>
        <div className="flex items-center gap-3 text-sm">
          <span className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/50 text-amber-700 rounded-xl font-bold shadow-sm backdrop-blur-sm transition-transform hover:scale-105">
          ...
          <span className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200/50 text-emerald-700 rounded-xl font-bold shadow-sm backdrop-blur-sm transition-transform hover:scale-105">
            <CheckCircle className="h-4 w-4 text-emerald-600" />
            {approvedWorks.length} Approved
          </span>
          <span className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-red-50 to-rose-50 border border-red-200/50 text-red-700 rounded-xl font-bold shadow-sm backdrop-blur-sm transition-transform hover:scale-105">
...
let updated = content.substring(0, startIndex) + newContent + content.substring(endIndex);
fs.writeFileSync(file, updated, 'utf8');
console.log('Successfully updated TravelAllowanceTab.tsx');
