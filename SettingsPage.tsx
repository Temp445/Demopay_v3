 ...
 *
 * Comprehensive settings management interface with SMTP configuration
 */
...
export default function SettingsPage() {
  ...
  const validateForm = (): boolean => {
    ...
    } else if (smtpConfig.password.length < 6) {
  ...
  const handleInputChange = (field: keyof SMTPConfiguration, value: string | number) => {
    setSmtpConfig(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleTestConnection = async () => {
    try {
      ...
      // that safely tests the SMTP connection server-side
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center">
          <Mail className="h-7 w-7 mr-3 text-indigo-600" />
          SMTP Configuration
        </h1>
