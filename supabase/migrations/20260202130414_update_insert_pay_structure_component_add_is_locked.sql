/*
  # Update insert_pay_structure_component Function to Include is_locked

  1. Changes
    - Add p_is_locked parameter to function signature
    - Update INSERT statement to include is_locked field
    - Default value is false for backward compatibility

  2. Purpose
    - Allow saving the is_locked flag when creating/updating salary structures
    - Support the new simplified locking mechanism
*/

-- Drop the existing function
DROP FUNCTION IF EXISTS insert_pay_structure_component(
  numeric, text, text, uuid, text, text, boolean, numeric, text[], uuid, uuid, boolean, boolean
);

-- Recreate the function with is_locked parameter
