/*
  # Update insert_pay_structure_component Function to Include is_applied_in_calculation

  1. Changes
    - Add p_is_applied_in_calculation parameter to function signature
    - Update INSERT statement to include is_applied_in_calculation field
    - Default value is true for backward compatibility

  2. Purpose
    - Allow saving the is_applied_in_calculation flag when creating/updating salary structures
    - Support the checkbox functionality for statutory deductions
*/

-- Drop the existing function
DROP FUNCTION IF EXISTS insert_pay_structure_component(
  numeric, text, text, uuid, text, text, boolean, numeric, text[], uuid, uuid, boolean, boolean, boolean
);

-- Recreate the function with is_applied_in_calculation parameter
