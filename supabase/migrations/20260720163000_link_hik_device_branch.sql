-- Migration: link_hik_device_branch
...
-- 2. Add latitude and longitude to attendance_timestamp
...
BEGIN
    ...
    IF NEW.device_ip IS NOT NULL THEN
        ...
        IF v_branch_location_id IS NOT NULL THEN
            SELECT CAST(elem->>'latitude' AS NUMERIC), CAST(elem->>'longitude' AS NUMERIC)
    ...
    INSERT INTO public.attendance_timestamp (employee_id, entry, "timestamp", timing_status, tenant_id, shift_id, latitude, longitude) 
    VALUES (v_employee_uuid, v_new_entry, NEW.event_time, v_timing_status, NEW.tenant_id, v_shift_id, v_latitude, v_longitude);
